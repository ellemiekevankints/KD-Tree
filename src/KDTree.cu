...
/* 
* will need to return void for CUDA implementation
* may need to pass in AutoBuffer + PQueue (?)
*/
template<typename T> 
KDTree<T>::DMatch KDTree<T>::findNearest(KDTree<T> kdtree, ssrlcv::Feature<T> feature, int k, int emax) const {

    T desc = feature.descriptor;
    const unsigned char *vec = desc.values; // descriptor values[128] from query

    AutoBuffer<unsigned char> _buf((k+1)*(sizeof(float) + sizeof(int)));
    ...
    AutoBuffer<unsigned char> _pqueue(maxqsize*sizeof(PQueueElem)); 
...
template<typename T>
const float2 KDTree<T>::getPoint(int ptidx, int *label) const {
...
int main() {
    ...
    KDTree<ssrlcv::SIFT_Descriptor> kdtree = KDTree<ssrlcv::SIFT_Descriptor>(img1);
    ...
    ssrlcv::Unity<KDTree<ssrlcv::SIFT_Descriptor>::DMatch>* dmatches = new ssrlcv::Unity<KDTree<ssrlcv::SIFT_Descriptor>::DMatch>(nullptr, numPossibleMatches, ssrlcv::cpu);
